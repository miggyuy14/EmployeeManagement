...
if (isset($_POST['Add'])) {
    ...
    $sql = "SELECT username FROM employees WHERE fingerprint_select=1";
    $result = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($result, $sql)) {
        echo "SQL_Error";
        exit();
            ...
            if ($row['username']=="Name") {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql="UPDATE employees SET username=?, employee_id=?, gender=?, email=?, user_date=CURDATE(), time_in=? WHERE fingerprint_select=1";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
...
if (isset($_POST['Add_fingerID'])) {
    ...
    if ($fingerid == 0) {
        ...
        if ($fingerid > 0 && $fingerid < 128) {
            $sql = "SELECT fingerprint_id FROM employees WHERE fingerprint_id=?";
            $result = mysqli_stmt_init($conn);
            if (!mysqli_stmt_prepare($result, $sql)) {
                echo "SQL_Error1";
                exit();
                ...
                if (!$row = mysqli_fetch_assoc($resultl)) {
                    ...
                    $sql = "SELECT add_fingerid FROM employees WHERE add_fingerid=1";
                    $result = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        echo "SQL_Error2";
                        exit();
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                mysqli_stmt_execute($result);
                                $sql = "INSERT INTO employees (username, employee_id, gender,  fingerprint_id, fingerprint_select, user_date, time_in, del_fingerid , add_fingerid) 
                                        VALUES (?, ?, ?, ?, 1, CURDATE(), ?, 0, 1)";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error4";
                                    exit();
...
if (isset($_POST['Update'])) {
    ...
    //check if there any selected user
    $sql = "SELECT * FROM employees WHERE fingerprint_select=1";
    $result = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($result, $sql)) {
        echo "SQL_Error";
        exit();
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email) && !empty($Timein)) {
                                ...
                                $sql="UPDATE employees SET username=?, employee_id=?, gender=?, email=?, time_in=? WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
                                ...
                                if (!empty($Timein)) {
                                    $sql="UPDATE employees SET gender=?, time_in=? WHERE fingerprint_select=1";
                                    $result = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result, $sql)) {
                                        echo "SQL_Error_select_Fingerprint";
                                        exit();
